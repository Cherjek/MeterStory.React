...
export class PulsMetersDataService extends Fetch {
  constructor() {
    super('/meter/data');
  }
...
function energy_response_parse(response) {
  ...
  for (let i = 0; i < response.measures[0].devices.length; i++) {
		for (let j = 0; j < response.measures[0].devices[i].vals.length; j++) {
			...
			{
        if ( response.measures[0].devices[i].vals[j].tags[k].tag === (`Pls${k+1}`) )
				 	meterData[`Pls${k}`] = response.measures[0].devices[i].vals[j].tags[k].val;
